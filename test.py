...
def add_watermark():
    ...
    system_name = os.name
    if system_name == 'nt':
        final_video = mp.CompositeVideoClip([video, logo1, logo2])
        final_video.write_videofile("D:\\bilibili\\" + get_video_file() +
        ...
        final_video = mp.CompositeVideoClip([video, logo1, logo2])
        final_video.write_videofile(get_video_file() +
                                    "_combined_watermark.mp4",
                                    audio_codec='aac',
                                    threads=32,
                                    verbose=False,
...
# video_file = get_video_file()
# if not video_file:
#     link = "https://www.youtube.com/watch?v=NozeAGkT5sY"
#     download_video(link)
# energy = show_graph()
# toCombine = "No"
# regenerate = True
...
def generate_highlights():
    # video_file = get_video_file()
    # print("Target Video File %s" % video_file)
    # if not video_file:
    #     link = "https://www.youtube.com/watch?v=BXFOZv79Rnk"
    #     download_video(link)
    # energy = show_graph()
    # thresh, duration = calculate_thres_value(energy)
    # generate_clips(energy, float(thresh), float(duration))
